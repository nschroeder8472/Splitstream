...
## 2026-07-24 — profiles, full implementation
- **Scope**: 14 files across engine/control/app layers (369 total workspace tests)
- **Atoms**: clean-code, architecture, secure-coding, test-quality (mixed-layer diff, config-file trust boundary, new test files)
- **Result**: 0 critical, 1 warning (fixed), 2 suggestions (both noted, not applied)
- **Key findings**: `spawn_hotkeys`'s per-binding-isolation doc comment was false for chord-conversion failure — `to_global_hotkey(chord)?` aborted the whole function on the first bad chord, silently dropping every other hotkey too; latent (currently unreachable given `HotkeyChord::parse`/`key_code`'s key sets agree), fixed by skipping individually. Suggestions: `apply_profile_action`'s ~65-line mixed-path dispatch could extract a helper; "Save As" onto an existing profile name silently overwrites with no warning (unaddressed edge case, not a new pattern — matches this codebase's existing no-confirm-on-destructive convention).
- **Strengths**: `control::profiles::apply_profile` diffs target against *live* state field-by-field rather than unconditionally emitting all seven per-group edits — the one piece genuinely easy to get wrong, and it's what makes `capture_then_apply_is_a_no_op` a real no-op and keeps a gain-only switch off the `Structural` rebuild path; all 8 L4 test contracts present and passing.

## 2026-07-24 — external-controls, full implementation
- **Scope**: 16 files (14 modified + 2 new) across engine/win-audio/control/app layers, ~1450 lines (397 total workspace tests)
- **Atoms**: clean-code, architecture, secure-coding, test-quality (mixed-layer diff, new COM adapter, config-file trust boundary, new test files)
- **Result**: 0 critical, 2 warning (both fixed), 0 suggestions
- **Key findings**: `Dispatcher::set_current` (already called on every param edit, including every frame of a fader drag — egui's `Slider::changed()` fires per-frame, no debounce upstream) gained an unconditional `tray_handle.rebuild()` call, meaning a fader drag would tear down and recreate the native tray menu dozens of times a second; fixed with a `PartialEq` before/after comparison that skips the rebuild when nothing tray-relevant changed. `handle_endpoint_volume_changed` unconditionally emitted both a gain *and* a mute edit on every inbound event (`reconcile`'s `AdoptFromEndpoint` always bundles both fields even when only one differs), so a plain volume-key press also pushed a redundant same-value `SetMute` back to the real COM object; fixed with a proper before/after diff per field — third occurrence of the "diff before emit, don't trust a bundled result's presence" pattern this session block (see profiles' review).
- **Strengths**: the COM adapter (`endpoint_volume.rs`) was validated against real hardware mid-implementation (`level=0.06 muted=false` read from the actual default device), not left as an untested guess; the coordinator's synthetic re-adopt (`adopt_now`) correctly unifies flow A's bind-time adopt and flow D's leaving-suspended adopt into one mechanism, with a dedicated regression test for the "rebind with an unchanged suspended value must still re-adopt" edge case; `HotkeyChord`'s `char`-only key was widened to a `HotkeyKey` enum after noticing the design doc's own example config (`Space`/`Up`/`Down`) couldn't have parsed under the original type.

## 2026-07-24 — visual-identity, full implementation
- **Scope**: 8 files modified + 1 new (`app/src/theme.rs`) across engine/control/app layers (426 total workspace tests)
- **Atoms**: clean-code, architecture, secure-coding, test-quality (mixed-layer diff, config-file trust boundary, new test files)
- **Result**: 0 critical, 2 warning (both fixed), 1 suggestion (fixed)
- **Key findings**: `store.rs`'s `SetTheme`/`SetAccent` repeated the exact `[app]`-table-lookup block `SetAutostart`/`SetActiveProfile` already used, pushing one pattern to 4 copies — extracted a shared `app_table(doc)` helper. `theme.rs`'s `semantic_colours_do_not_vary_with_the_accent` compared two identical calls to a function with no accent parameter — trivially true regardless of correctness, since the real guarantee is the missing parameter (compile-time); replaced with a real assertion that dark/light actually differ. Minor: `crate::theme::` prefix inconsistent with the rest of the file's plain `theme::`.
- **Strengths**: three design-doc-named tray-icon refresh triggers (startup, menu rebuild, opportunistic-while-open) collapsed into one mechanism riding the dispatcher's existing 50ms tick with a real before/after comparison, rather than three separate call sites; `install`'s contract cited an egui `set_style` call that doesn't exist in 0.35 — verified against the local registry source and implemented via `set_visuals_of` + `all_styles_mut`, applying only the shared radius/spacing fields so the just-installed per-theme accent colours survive; all 8 Level-4 test contracts present and passing, contrast-checked accent presets backed by a hand-computed WCAG ratio (not eyeballed).

## 2026-07-25 — audio-flow-control, full implementation
- **Scope**: 6 files across engine/win-audio/app — ports interface, platform adapter, orchestration, pure control layers (313 total workspace tests)
- **Atoms**: clean-code, architecture, test-quality (DDD/secure-coding skipped — no domain/trust-boundary touch)
- **Result**: 1 warning — **fixed same session**
- **Key findings**: a new flow-control regression test (`render_loop_pops_only_what_the_device_will_accept`) synchronized two threads via two flat `sleep(20ms)` calls instead of this same file's own established poll-until-deadline idiom, despite this blueprint's own decision 4 explicitly rejecting wall-clock test pacing by name for flakiness reasons; fixed by removing the first (dead-weight, given the paced mock's counter-based release) and replacing the second with a bounded poll on the real observable effect (`device.filled_frames() > 0`).
- **Strengths**: the drift controller's sign flip (B7) was pinned against `Src`'s *measured* real output-frame counts at two ratios, not re-derived from a reading of `resample.rs` — same self-skepticism this codebase's operational learnings have called for twice before; the mixer governor's per-group/per-output headroom check was grounded against `mixer.rs`'s actual summing behavior before designing to it, correctly ruling out a cross-output starvation failure mode a naive single-budget design would have had; 8 new regression tests directly exercise B1/B2/B3/B4/B8 rather than re-asserting the design doc's own claims.

## 2026-07-25 — routing-truthfulness, full implementation
- **Scope**: 10 code files across engine/control/app (rules/graph/routing, store/config/profiles, main/ui/event_pump), ~601 insertions/134 deletions
- **Atoms**: clean-code, architecture, DDD, secure-coding, test-quality (all layers touched, config file I/O, tests included)
