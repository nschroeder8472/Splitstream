...
Dropped: P6 own driver (spec §13). Won't ship — Microsoft driver-signing (EV cert + attestation, legal entity, recurring cert) is bureaucratic overkill for free open-source with no revenue. BYOD (spec §9.5, resolved 2026-07-20) is the permanent design, not a stopgap: no bundled/installed driver at all — the user supplies their own (free VB-CABLE recommended in onboarding docs), Splitstream detects it by configurable name-prefix bus classification. Revisit only if native branded endpoints or capture-device exposure (spec open question #6, OBS/Discord) ever become must-haves.
